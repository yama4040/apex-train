...
class DirectRewardPredictor:
    def __init__(self, model_path='classification_reward_model.h5', scaler_path='classification_reward_scaler.pkl'):
    ...
    def predict_reward(self, state_info):
        ...
        try:
            ...
            # ▼▼▼ 【重要修正】分類モデルからの報酬復元 ▼▼▼
            # 最も確率の高いクラス（0〜10）のインデックスを取得
            best_class_index = int(tf.argmax(preds, axis=1).numpy()[0])
            
            # 0.1 を掛けて元の 0.0 〜 1.0 のスケールに戻す
            rounded_reward = best_class_index * 0.1
