...
def load_and_preprocess_data(csv_dir):
    """train_reward_csv_direct 内の全CSVを読み込み、学習時と同一の特徴量に変換する"""
    ...
    return df, X, feature_cols, llm_reward
...
def main():
    df, X, feature_cols, llm_reward = load_and_preprocess_data(CSV_DIR)
    ...
    if reg_model is not None:
        print("回帰NNで推論中...")
        raw_reg, rounded_reg = predict_regression(reg_model, reg_scaler, X)

    rounded_cls = None
    if cls_model is not None:
        print("分類NNで推論中...")
        _, rounded_cls = predict_classification(cls_model, cls_scaler, X)

    # 実装内容②: 学習データ全体でのLLM分布とNN分布の比較
    plot_full_distribution_comparison(
        llm_reward, rounded_reg, rounded_cls,
        save_path=os.path.join(OUTPUT_DIR, "nn_vs_llm_distribution_all.png")
    )

    # 実装内容①: LLMラベルが0.0の入力に対するNN出力の実態
    plot_zero_label_output(
        llm_reward, raw_reg, rounded_reg, rounded_cls,
