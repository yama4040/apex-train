...
class Actor:
    ...
    def evaluate_gammas_with_local_llm(self, log_csv, base_gamma, total_steps):
        ...
        try:
            ...
            if self.max_important_steps > 0:
                ...
                0.9000 ～ 1.0000 の範囲でガンマ値を付与してください。
                ...
                mode_instruction = f"""
                提供された全ステップ（計 {total_steps} 個）に対して、それぞれ 0.9000 ～ 1.0000 の範囲でガンマ値を決定してください。
            ...
            提供された【走行ログ(CSV形式: 残り距離(km), 速度(km/h), 行動)】を分析し、省エネや定時到着に貢献した行動を特定し、
            TQ値の計算に使用する割引率（ガンマ）を決定してください。
            ガンマの値は、行動の重要性に応じて0.9000から1.0000の範囲で決定してください。
            ガンマの値により将来の報酬が割り引かれる（減衰する）ため、重要な行動には1.0000に近いガンマを、そうでない行動には0.9000に近いガンマを付与してください。
            TQ値は「報酬 + ガンマ * 次の状態での最大Q値」で計算されることを考慮し、ガンマの値を決定してください。
            {mode_instruction}
    ...
    def rollout(self, current_weights):
        ...
        # ▼▼▼ 【修正】CSVの1行目（ヘッダー）として日本語の列名をセットしておく ▼▼▼
        episode_logs = ["残り距離(km),速度(km/h),行動"]
        # ▲▲▲ ここまで ▲▲▲
        
        # ▼▼▼ 【修正】ここで step_count を初期化します ▼▼▼
        step_count = 0
        # ▲▲▲
        ...
        while not done:
            ...
            # environment.py の実装に従い、残り距離(km)を取得
            ...
            # ▼▼▼ 【修正】残り距離、速度、行動の3つだけをCSV記録 ▼▼▼
            action_ja = action_map.get(int(action), str(int(action)))
            csv_line = f"{remain_dist:.3f},{current_speed:.1f},{action_ja}"
            # ▲▲▲
       ...
       # =========================================================
        # LLMによる動的ガンマの取得とバッファの上書き
        # =========================================================
        self.episode_count += 1
        log_csv_for_llm = "\n".join(episode_logs)
        total_steps = len(self.buffer)
        
        # ▼▼▼ 【修正】 self.gamma ではなく self.__gamma (生の0.9975) を渡す ▼▼▼
        llm_gammas = self.evaluate_gammas_with_local_llm(log_csv_for_llm, self.__gamma, total_steps)
        # ▲▲▲ ここまで ▲▲▲
        
        
        # バッファ内のタプルを一度リスト化して上書きし、タプルに戻す
        for i in range(total_steps):
            adjusted_llm_gamma = float(llm_gammas[i]) ** (self.env.time_step / self.time_step)
            temp_transition = list(self.buffer[i])
            temp_transition[6] = adjusted_llm_gamma
...
if __name__ == "__main__":
    ...
    main(num_actors=2, gamma=0.9500, num_states=9, time_step=1.0)
