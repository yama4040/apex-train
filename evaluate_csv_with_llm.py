...
# 出力ルール
--------------------------------------------------
以下のStep1〜Step8の順に、必ずすべてのStepを内部で確認すること。
...
Step1〜Step7それぞれについて、"OK"（問題なし）／"NG"（問題あり。10文字程度の短い補足を括弧内に付けてよい）／"対象外"（評価対象外）
...
"""
    output_format = """
# 出力指示
"checks"には、Step1〜Step7それぞれの判定（"OK"／"NG(補足)"／"対象外"）を必ずすべて記載すること（キーの省略不可）。
"reason"には、Step1〜Step7の説明を並べるのではなく、rewardを決定づけた要因のみを50〜100文字程度の一文で簡潔に記述すること。
...

{
  ...
  "reason": "ノコギリ運転に該当し、かつ定時性に余裕があるため力行継続は省エネ性に欠けると判断し0.5とする。",
  "reward": 0.5
}
"""
