...
- 高評価：delta_stop >= 0m（このタイミングでブレーキをかけると駅手前に停車する可能性があるためブレーキをかけていないのは適切です）
  ※ただし、先行列車も無く、遅延しているのにもかかわらず（delay > 0）低速度（current_speed < 20)で惰行をして停止位置へ進む動きは、遅延を増大させる動きであるためreward=0.0点としてください。
- やや減点：-2m モなくelta_stop <0m 
...
- 先行列車が在線しておらず、遅延が生じているのにも関わらず低速度で惰行をする行為（詳しくは減速フェーズの評価ルールを参照）

--------------------------------------------------
# 出力ルール
--------------------------------------------------
以下のStep1〜Step8の順に、各Stepごとに必ず推論を行い、その理由をStepごとに出力すること。
一つのStepの結論が後続のStepの前提になる場合は、その内容を踏まえて次のStepの推論を行うこと。
...
Step7 定時性・省エネ性評価
Step8 総合評価（Step1〜Step7を踏まえたrewardの最終判断）
...
"""
    output_format = """
# 出力指示
reasonには、Step1〜Step8それぞれについて順番に推論した理由を、下記のように「Step番号(項目名): 理由」の形式で必ず全Step分出力すること。Stepを省略したり、まとめて記述したりしてはならない。
各Stepの理由は20〜40文字程度、reason全体では300〜500文字程度を目安とすること。
rewardは0.0～1.0（0.1刻み）で出力し、Step8（総合評価）の結論と整合させること。

{
  "reason": "Step1(制限速度): 制限70km/hに対し65km/hで走行中であり超過なし。Step2(CBTC): 信号現示70km/hを超えておらず問題なし。Step3(停止位置): 巡航フェーズのため評価対象外。Step4(フェーズ評価): 巡航フェーズだが必要速度未達のため力行継続は妥当。Step5(ノコギリ運転): 直前操作から2.0秒で惰行に切り替えており該当。Step6(前後列車間隔): 先行・後続列車ともに存在せず評価対象外。Step7(定時性・省エネ性): 定時到着に余裕がある中での力行継続は省エネ性に欠ける。Step8(総合評価): ノコギリ運転と省エネ性の観点から減点し、安全性は保たれているため0.5とする。",
  "reward": 0.5
}
"""
