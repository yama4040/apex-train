...
- 高評価（reward=0.8～1.0）：|delta_stop| ≤ 2m
- やや減点（reward=0.5～0.7）：2m < |delta_stop| ≤ 5m
- 減点（reward=0.2～0.4）：5m < delta_stop ≤ 15m（ブレーキ開始が早すぎる。誤差が小さいほど0.4に近づけること）
- 大幅減点（reward=0.1）：delta_stop > 15m（大幅に手前に停止することがほぼ確実）
...
- 高評価：delta_stop >= 0m（このタイミングでブレーキをかけると駅手前に停車する可能性があるためブレーキをかけていないのは適切です）
  ...
  （高評価に値するのは、停止位置に向けて適切なタイミング（|delta_stop| ≤ 5m）でブレーキをかけて停止する運転のみである）
...
- 1m < |dist_to_next_station| ≤ 3m → reward = 0.7
- 3m < |dist_to_next_station| ≤ 5m → reward = 0.4
...
- ちんたら運転（低速のまま停止位置へ進む運転）→ reward=0.1～0.3
