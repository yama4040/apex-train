...
- 制限速度未満での力行（加速）継続 → 高評価（遅延回復のため）。制限速度に対し十分低いのに惰行・減速しているのは
  遅延を拡大させる運転として減点する（速度が低いほど・長く続くほど強く減点）。
- 惰行からの再加速（力行）を高評価とする条件（いずれかを満たせばよい）:
  - 現在速度が「制限速度 − 10km/h」以下まで低下している
  - 現在の惰行（同一ノッチ）の継続時間が7秒以上経過している（＝ノコギリ運転にならない）
- 制限速度に到達しそうな局面での惰行 → 高評価（超過回避の適切な惰行）。
  ただし「制限速度 − 10km/h」を大きく下回ってなお惰行を続けるのは、回復機会の損失として減点する。
- 制限速度を超える方向の走行は不可（超過は下記「即reward0.0ルール」で0.0。超過前のブレーキ・惰行は高評価）。
