...
- 条件: ダイヤ維持のために制限速度付近までの力行が必要な状況（**required_speed が speed_limit − 3km/h 以上**、
　＝定時到達には制限速度近くで走り続ける必要がある）、かつ駅間停車防止に該当しない。
　※current_status の「計画ダイヤに対する遅延（delay）」は、標準運転時間を超過して初めて正になる遅行指標で
　　走行中は0のことが多い。走行中の判定は **required_speed が制限速度付近かどうか**を主に用いること
　　（delay > 0 はより確実な補助指標）。
- 基準速度: **speed_limit（制限速度）**。省エネ性より遅延回復を優先し、制限速度付近まで力行して
　速度を維持する（通常運転モードの「早めの惰行＝高評価」とは逆に、制限速度付近までの力行継続を高評価する）。
...
# フェーズ別評価基準
--------------------------------------------------
※【モードによる読み替え】以下のフェーズ別基準は通常運転モードを前提に記述している。
...
　適用せず、過剰加速として減点する。
　mode = "delay_recovery"（遅延回復モード）の場合は、基準速度を speed_limit（制限速度）に読み替え、
　矛盾する場合は必ず「# 遅延回復モードの評価基準」を優先すること。特に巡航フェーズの「必要速度を満たせば
　惰行を高評価」は遅延回復モードでは適用せず、制限速度付近までの力行継続を高評価とする（早すぎる惰行は減点）。
...
先行列車が居る場合
- current_speed ≒ signal_speed （2つの速度の差が±2km/h以内）かつ ブレーキ中
  → 高評価（信号現示に整合して減速しており，駅の手前に停止する可能性があっても現示に従っているため）
- current_speed > signal_speed ＋2km/h（CBTC現示を明確に超過）
  → **ノッチに関わらず reward=0.0**。ブレーキ中であっても，現示を大きく超えた速度で先行列車へ接近していること自体が
    CBTC違反であり危険なため必ず0.0とする。過剰加速で現示を超えた後にブレーキをかけても，その超過状態は0.0である。
...
※このとき target_speed_no_stop は**観測遅延を織り込んで低く算出されている**（先行がまだ停車中である分、
　機外停車を避ける加速上限も低い）。よって基準速度は通常どおり target_speed_no_stop を用いてよく、
　観測遅延がある局面では特に、target_speed_no_stop を超える加速を「先行に追いつく過剰加速」として明確に減点すること。
...
--------------------------------------------------
駅出発時点で30秒以上遅延しており、先行に塞がれていない状況。省エネより遅延回復を優先し、
制限速度（speed_limit）付近まで力行して速度を維持する。基準速度は required_speed ではなく speed_limit。
評価はあくまで現在のノッチ操作に対して行う。

【基本方針】制限速度を超えない範囲でできる限り高い速度を維持することが目的である。そのために
**「制限速度 − 8km/h ～ 制限速度 − 2km/h」の帯（維持帯）を、力行と惰行を交互に繰り返して保つ**のが
理想の運転である。上端（制限速度 − 2km/h）に達したら惰行に移り、下端（制限速度 − 8km/h）まで
落ちたら再び力行する。この帯幅であれば、力行相・惰行相ともに7秒以上continueするため
ノコギリ運転にはならない（平坦で惰行約26秒・力行約12秒）。乗り心地はこの周期の長さで担保する。

## 加速・巡航フェーズ（平坦・上り勾配、または −6‰以浅のゆるい下り勾配）

### 維持帯の中にいるとき（速度が「制限速度 − 8km/h」～「制限速度 − 2km/h」）
- **力行の継続も、惰行の継続も、どちらも同等に高評価とする。**
  帯を保てている状態そのものが遅延回復の成功であり、力行相と惰行相はその一周期を構成する両輪である。
- **【重要】惰行を力行より低く評価してはならない。** 帯の上端側で惰行しているのは「これ以上加速すると
  制限速度を超えるため速度を維持している」正しい運転であり、力行と同じ水準の高評価を与えること。
  ここで惰行を低く評価すると、「わざと減速して力行できる時間を作る」運転のほうが有利になってしまう。
- 帯の中でのブレーキ（減速）は、速度を落として遅延を拡大させるため減点する
  （駅停車・CBTC・先行列車への対応など正当な理由がある場合を除く）。

### 上端に達したとき（速度が「制限速度 − 2km/h」以上）
- **惰行 → 高評価**（超過を防ぎつつ高い速度を保つ正しい操作）。
- **力行 → 減点。** 1秒あたり約0.5km/h加速するため、ここで力行を続けると制限速度を超過して即0.0になる。
  制限速度に近いほど0.1に近づけること。

### 下端を下回ったとき（速度が「制限速度 − 8km/h」より低い）
- **力行（加速）→ 高評価**（帯へ戻す正しい操作）。ただし後述の「自ら落とした直後の再加速」を除く。
- 惰行・減速の継続 → 減点（遅延を拡大させる。速度が低いほど・長く続くほど強く減点する）。

### 【重要】自ら不要に減速した直後の再加速は高評価としない
- 直前がブレーキ（prev_notch がブレーキ）で速度を落としており、かつその減速に正当な理由
  （駅への停車、CBTC現示の低下、先行列車への接近、制限速度の超過回避、急な下り勾配）が**無い**場合、
  その後の再加速は**高評価としない（0.4程度を上限とする）**。
- 理由: 不要な減速そのものを減点しても、その後に続く長い再加速を高評価にしてしまうと、
  「一度大きく減速してから加速し直す」運転のほうが累積で得になり、加速と急減速を繰り返す
  運転を誘発するため。正当な理由なく速度を落とした場合は、その回復も評価しない。
- 一方、正当な理由で減速した後の再加速（駅発車後、先行列車の解放後、制限区間の通過後など）は
  従来どおり高評価とする。

### 超過とその復帰
- 制限速度を超える方向の走行は不可（超過は下記「即reward0.0ルール」で0.0）。
- **【超過してしまった後の復帰】制限速度をわずかに超過した状態からの復帰は、惰行で穏やかに速度を戻すのが
  正しい操作であり高評価とする**（惰行なら数秒で制限内に戻る）。この局面での強いブレーキは、
  速度を大きく落としすぎて遅延を拡大させ乗り心地も損なうため減点する
  （ただし制限を大きく超過している場合や、駅・先行列車への停止が必要な場合のブレーキは適切であり減点しない）。

### 急な上り勾配で加速できない場合
- 上り勾配が急（概ね +6‰ 以上）で、力行しても速度が上がらない、または上端に届かない場合は、
  **力行を継続し続けることが正しい**（惰行に移ると速度が大きく落ちてしまうため）。
  この場合に力行が長時間continueすることをノコギリ運転や過剰加速として減点してはならない。

## 急な下り勾配（current_gradient が −6‰ より急な下り。惰行でも速度が上がり得る）
- **力行（加速）は不要・減点**（重力で制限速度を超える危険。惰行が7秒以上経過していても、急な下り勾配では加速しない）。
- **維持帯（制限速度 − 8km/h ～ − 2km/h）での惰行を最高評価**（重力で速度が落ちにくいため、
  急な下りでは惰行のまま高い速度を保てる。これが理想の運転である）。
- 速度が制限速度へ近づく場合 → 超過する前のブレーキを高評価。

## 勾配の終了を考慮したブレーキ（前方の勾配情報を参照）
- 下り勾配の終了地点が近い（前方の勾配情報が「Xm先で下り勾配…が終わり平坦になる」等）局面で、
  過度に減速するのは減点する。勾配が終わると平坦部で再加速が必要になりエネルギー効率が悪いため、
  勾配が終わることを見越して減速しすぎないことが望ましい。

## 乗り心地
- ノコギリ運転ルール（7秒・反転・合理的理由なし）はそのまま適用する。
  維持帯（制限速度 − 8km/h ～ − 2km/h）は、力行相・惰行相のいずれも7秒以上continueするように
  帯幅を定めてある。したがって帯の端で切り替えている限りノコギリ運転には該当しない。
  逆に、帯の端に達していないのに力行と惰行を短い周期で切り替えているものは
  ノコギリ運転として減点する。
...
## 評価ルール
- current_speed > signal_speed ＋2km/h（CBTC現示を明確に超過）
  → **ノッチに関わらず reward=0.0**。現示を大きく超えた速度で走行していること自体がCBTC違反（先行列車への
    接近超過）であり危険なため必ず0.0とする。
    ※「ブレーキで信号現示に追随している」として高評価にしてよいのは，下記の current_speed ≒ signal_speed
      （±2km/h以内）の場合に限る。過剰加速で現示を大きく超えた後にブレーキをかけても，その超過状態は0.0である
      （早め惰行で現示を超えないようにすべきだったため）。
- current_speed ≒ signal_speed （2つの速度の差が±2km/h以内）かつ ブレーキ中
  → 高評価（信号現示に整合して減速しており，駅の手前に停止する可能性があっても現示に従っているため）
...
- 制限速度超過
- signal_speed超過（current_speed > signal_speed ＋2km/h。**ノッチに関わらず**。ブレーキ中でも現示超過は0.0）
- オーバーラン走行中
