...
"""
    output_format = """
# 出力指示
reasonには、Step1〜Step8それぞれについて順番に推論した理由を、下記のように「Step番号(項目名): 理由」の形式で
改行区切り（\\n）で必ず全Step分出力すること。Stepを省略したり、まとめて記述したりしてはならない。
各Stepの理由は20〜40文字程度、reason全体では300〜500文字程度を目安とすること。
rewardは0.0～1.0（0.1刻み）で出力し、Step8（総合評価）の結論と整合させること。

{
  "reason": "Step1(制限速度): 制限70km/hに対し65km/hで走行中であり超過なし。\\nStep2(CBTC): 信号現示70km/hを超えておらず問題なし。\\nStep3(停止位置): 巡航フェーズのため評価対象外。\\nStep4(フェーズ評価): 巡航フェーズだが必要速度未達のため力行継続は妥当。\\nStep5(ノコギリ運転): 直前操作から2.0秒で惰行に切り替えており該当。\\nStep6(前後列車間隔): 先行・後続列車ともに存在せず評価対象外。\\nStep7(定時性・省エネ性): 定時到着に余裕がある中での力行継続は省エネ性に欠ける。\\nStep8(総合評価): ノコギリ運転と省エネ性の観点から減点し、安全性は保たれているため0.5とする。",
  "reward": 0.5
}
"""
