...
【基本方針】ノコギリ運転でなく、かつ制限速度を超えていなければ、力行（加速）を優先し高評価する。
惰行が適切なのは「制限速度に到達しそうで超過を回避する場合」または「下り勾配を活用する場合」に限る。

## 加速・巡航フェーズ（平坦・上り勾配、または −6‰以浅のゆるい下り勾配）
遅延回復の運転は「**制限速度 − 3km/h ～ 制限速度 − 10km/h の帯を保つ**」ことを理想とする。
帯の上端で惰行に移り、下端で再加速する（1秒あたり約0.5km/h加速するため、上端より上で力行を続けると
制限速度を超過して即0.0になる）。

- 現在速度が「制限速度 − 10km/h」～「制限速度 − 3km/h」の範囲での力行（加速）継続 → 高評価（遅延回復のため）。
  制限速度に対し十分低い（− 10km/h より下）のに惰行・減速しているのは、遅延を拡大させる運転として
  減点する（速度が低いほど・長く続くほど強く減点）。
- 惰行からの再加速（力行）を高評価とする条件（いずれかを満たせばよい）:
  - 現在速度が「制限速度 − 10km/h」以下まで低下している
  - 現在の惰行（同一ノッチ）の継続時間が7秒以上経過している（＝ノコギリ運転にならない）
- **【重要・超過の予防】現在速度が「制限速度 − 3km/h」以上での力行（加速）は、次の数秒で制限速度を
  超過させる過剰加速として減点する**（超過幅が近いほど0.1に近づける）。この帯では惰行が正しい操作であり、
  「制限速度 − 3km/h」以上での惰行 → 高評価（超過回避の適切な惰行）。
  ただし「制限速度 − 10km/h」を大きく下回ってなお惰行を続けるのは、回復機会の損失として減点する。
...
- **制限速度 − 5km/h 付近での惰行を最高評価**（重力で速度を維持しつつ、制限速度超過への余裕を確保する）。
...
## 乗り心地
- ノコギリ運転ルール（7秒・反転・合理的理由なし）はそのまま適用する（上記の「惰行7秒経過で再加速」条件はこれと整合）。
