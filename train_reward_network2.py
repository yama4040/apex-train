...
def build_model(input_dim):
    # 回帰器。出力は線形(0~1レンジへの収束は正則化・BN・Dropoutで促す)。
    l2_reg = l2(1e-4)
    model = Sequential([
...
def plot_learning_curve(history, out_path='learning_curve_direct.png'):
    ...
    plt.title('Model Loss (Huber)'); plt.ylabel('Loss'); plt.xlabel('Epoch'); plt.legend(loc='upper right')
    plt.subplot(1, 2, 2)
...
def main(csv_dir='train_reward_csv_direct',
    ...
    print("回帰器の学習を開始します...")
    history = model.fit(X_train_nz, y_train_nz, sample_weight=train_sample_weight_nz,
    ...
    with open(manifest_path, 'w', encoding='utf-8') as f:
        json.dump({'state_feature_cols': state_cols,
                   ...
                   'state_dim': state_dim,
                   'input_dim': state_dim + rf.MODE_DIM}, f, ensure_ascii=False, indent=2)
