# -*- coding: utf-8 -*-
"""
複数駅間版（羽前成田→白兎→蚕桑・3ノッチ）のLLM評価ランナー。

既存 `evaluate_csv_with_llm.py`（単一区間）も東西線版 `evaluate_csv_with_llm_multi.py` も
**一切変更しない**。プロンプト本文は `prompt_ymulti.py` に分離してあり、本スクリプトは
「CSVを読む → プロンプトを組む → LLMへ送る → 応答を検証して書き戻す」だけを行う。

    入力  評価用csv_Yamagata/*.csv       （generate_eval_csv_ymulti.py の出力）
    出力  評価済ログ_Yamagata/llm_evaluated_dataset_ymulti.csv   （mode/reward/reason/checks 付き）
          評価済ログ_Yamagata/llm_evaluation_failed_rows.csv     （評価に失敗した行の退避先）

【重要】既存の `評価用csv/` `train_reward_csv_direct/` とは絶対に混ぜないこと。
プロンプト世代の混在は過去にラベル矛盾の実害を出している。

使い方:
    python evaluate_csv_with_llm_ymulti.py --dry-run            # APIを呼ばずプロンプトを検証
    python evaluate_csv_with_llm_ymulti.py --limit 20           # 20行だけ試す
    python evaluate_csv_with_llm_ymulti.py --workers 6          # 本番（並列6）
    python evaluate_csv_with_llm_ymulti.py --resume             # 中断した続きから
"""
import os
import re
import csv
...
import prompt_ymulti as PY
import reward_features_ymulti as rf

load_dotenv()

IN_DIR = CFG.EVAL_CSV_DIR
OUT_DIR = CFG.EVALUATED_DIR
OUT_NAME = "llm_evaluated_dataset_ymulti.csv"
...
# CSVの文字列 → float に直す列
NUMERIC = {
    "time", "position", "holding_time", "prev_notch_duration",
    ...
    "delay", "total_delay", "total_remaining_distance", "total_remaining_time",
...
def row_to_features(row):
    ...
    for k in NUMERIC:
        v = f.get(k)
        f[k] = float(v) if v not in (None, "") else 0.0
    f["is_dwelling"] = bool(f.get("is_dwelling", 0) >= 0.5)
    f["stations_remaining"] = int(float(row.get("stations_remaining") or 0))
    f["forward_departed_next"] = row.get("forward_departed_next") or None
    missing = [k for k in PY.FEATURE_KEYS if k not in f]
    if missing:
...
def _call_once(prompt_text):
    ...
    r = client.chat.completions.create(
        ...
        messages=[
            {"role": "system",
             "content": "あなたは列車の自動運転制御を評価するエキスパートです。"
                        "必ず指示されたJSONフォーマットのみを出力してください。"},
...
def _validate(d, row):
    ...
    mode = d.get("mode")
    if mode not in PY.MODES:
        return f"mode が不正: {mode!r}（許可: {PY.MODES}）"
    # 本データセットに後続列車は存在しないので spacing は選ばれてはならない
    if mode == "spacing":
        return "mode=spacing は本データセットでは不正（後続列車が存在しない）"
    # 停車中／走行中とモードの整合
    dwelling = float(row.get("is_dwelling") or 0) >= 0.5
    if dwelling and mode != "hold_at_station":
        return f"駅停車中の行なのに mode={mode}（hold_at_station であるべき）"
    if not dwelling and mode == "hold_at_station":
        return "走行中の行なのに mode=hold_at_station"
    try:
    ...
    lack = [k for k in PY.CHECK_KEYS if k not in ch]
    if lack:
...
def evaluate_row(row, max_retries=3, dry_run=False):
    """1行を評価して (mode, reward, reason, checks, error, prompt) を返す。"""
    prompt = PY.build_prompt(row_to_features(row))
    if dry_run:
...
def main():
    ...
    ap.add_argument("--sample", type=int, default=None,
                    help="全体からN行を等間隔で抽出（試験用。先頭N行だと偏るため）")
    ap.add_argument("--resume", action="store_true", help="既存の出力に無い行だけ処理する")
    ap.add_argument("--dry-run", action="store_true", help="APIを呼ばずプロンプトだけ検証する")
    a = ap.parse_args()

    rows = load_rows(a.in_dir)
    if not rows:
        print(f"'{a.in_dir}' にCSVがありません。"
              f"先に python generate_eval_csv_ymulti.py を実行してください。")
        return
    ...
    headers = (["source_file"] + [c for c in cols if c not in rf.LLM_OUTPUT_COLS]
               + rf.LLM_OUTPUT_COLS + ["checks"])
    ...
    if a.dry_run:
        ...
        for r in todo:
            if shown >= 2:
                break
            if shown == 0 and float(r.get("is_dwelling") or 0) < 0.5:
                continue      # 1件目は停車中の行を見せる
            ...
            print(f"{r['_src']} / run_id={r.get('run_id')} / t={r.get('time')}s / "
                  ...
                  f"停車中={r.get('is_dwelling')}")
            print(f"プロンプト長 {len(p)} 文字")
            print(p[p.index("# 現在の走行状況と運転操作"):][:1600])
            shown += 1
        print("=" * 90)
        ng = 0
        for r in todo:
            try:
                PY.build_prompt(row_to_features(r))
            except Exception as e:
                ng += 1
                if ng <= 5:
                    print(f"  ✗ {row_key(r)}: {e}")
        print(f"プロンプト生成: 成功 {len(todo)-ng} / 失敗 {ng}")
        n_dwell = sum(1 for r in todo if float(r.get("is_dwelling") or 0) >= 0.5)
        print(f"内訳: 走行中 {len(todo)-n_dwell} 行 / 駅停車中 {n_dwell} 行")
        return
    ...
    def work(r):
        ...
        base = [r["_src"]] + [r[c] for c in cols if c not in rf.LLM_OUTPUT_COLS]
    ...
    print(f"\n完了: 成功 {stat['ok']} / 失敗 {stat['ng']} / 所要 {(time.time()-t0)/60:.1f} 分")
    if modes:
        print("  モード分布: " + " / ".join(f"{k}={v}" for k, v in sorted(modes.items())))
    print(f"  出力: {out_path}")
    if stat["ng"]:
        print(f"  失敗行: {fail_path}（原因を確認して --resume で再実行できます）")
    print(f"\n次の手順: 出力CSVを {CFG.TRAIN_CSV_DIR}/ に置いて "
          f"python train_reward_network_ymulti.py を実行してください。")
